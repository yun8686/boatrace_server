...
async function runGetSchedule(context) {
  ...
  page.on("request", (req) => {
    if (
      ["stylesheet", "image", "font", "script"].indexOf(req.resourceType()) >= 0
    ) {
      req.abort();
    } else {
      req.continue();
  ...
  const urls = await page.evaluate(async () => {
    ...
    list.forEach((v) => {
      if (v.innerHTML == "出走表") {
        var li =
          v.parentElement.parentElement.parentElement.parentElement.children;
        var jouname = li[0].querySelector("img").alt;
        jouname = jouname.slice(0, jouname.length - 1);
        var days = li[li.length - 4].innerText.split(
